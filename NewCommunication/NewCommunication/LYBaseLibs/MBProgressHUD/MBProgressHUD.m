...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view maskFrame:(CGRect)frame animated:(BOOL)animated
{
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithViewByMaskFrame:view maskFrame:frame];
	[view addSubview:hud];
	[hud show:animated];
	return MB_AUTORELEASE(hud);
}
...
- (id)initWithViewByMaskFrame:(UIView *)view maskFrame:(CGRect)frame
{
    NSAssert(view, @"View must not be nil.");
