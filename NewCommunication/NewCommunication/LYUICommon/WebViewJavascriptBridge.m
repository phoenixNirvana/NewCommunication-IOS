...
- (void)registerHandler:(NSString *)handlerName handler:(WVJBHandler)handler {
    self.messageHandlers[handlerName] = [[handler copy]autorelease];
}
...
- (void)_sendData:(NSDictionary *)data responseCallback:(WVJBResponseCallback)responseCallback handlerName:(NSString*)handlerName {
    ...
    if (responseCallback) {
        ...
        self.responseCallbacks[callbackId] = [[responseCallback copy]autorelease];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    if (webView != _webView || self.loadJs) { return; }
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    if (webView != _webView || self.loadJs) { return NO; }
